...
# 4. 연속된 값 있을 경우 --> 통합
name = re.sub("[\.\/_]{2,}", '', name)  # 3번 조건에서 연속된 값도 앞자리에 나오면 1개로 바로 정리 가능

# 5. 이모지가 있을 경우 _ 로 치환 --> 4분만 더 할애해보자.
name = re.sub(emoji_pattern, "_",
              name)  # re.compile한 값을 -> _ 패턴으로 -> name 문자열에서 변경한다.

if name[0] in "._/":  # 3. 첫번째 특수문자 안되는 것 제거
    first = name.replace(name[0], '')
    name = first
